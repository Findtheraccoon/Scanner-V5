"""Tests de los 6 triggers candle-level 15M (Fase 4a).
...
class TestDecayWeight:
    ...
    def test_decay_schedule_matches_v4_2_1(self, age: int, expected: float) -> None:
...
class TestGeneralBehavior:
    ...
    def test_no_bb_disables_bb_dependent_triggers(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([hammer], bb_15m=None)
    ...
    def test_zero_range_candle_is_skipped(self) -> None:
        ...
        assert detect_candle_15m_triggers([flat]) == []

    def test_single_candle_only_age_zero_fires(self) -> None:
        # Solo 1 vela → solo age=0 se chequea.
        candles = [_candle(o=100.0, h=100.2, low=98.0, c=100.1)]
        triggers = detect_candle_15m_triggers(candles, bb_15m=_bb(lower=98.0))
        assert all(t["age"] == 0 for t in triggers)
...
class TestDojiBB:
    def test_doji_bb_sup_fires_when_near_upper_band(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([doji], bb_15m=_bb(upper=110.0))
    ...
    def test_doji_bb_inf_fires_when_near_lower_band(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([doji], bb_15m=_bb(lower=90.0))
        assert any(
    ...
    def test_doji_ignored_when_body_too_large(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([big_body], bb_15m=_bb(upper=110.0))
    ...
    def test_doji_ignored_when_far_from_band(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([far], bb_15m=_bb(upper=110.0, lower=90.0))
        assert not any("Doji" in t["d"] for t in triggers)
...
class TestRechazo:
    def test_rechazo_sup_fires_at_age_0(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([candle])
    ...
    def test_rechazo_with_decay_at_age_3(self) -> None:
        # age=3 → decay=0.85, weight = 2 * 0.85 = 1.7
        rechazo = _candle(o=100.0, h=105.0, low=99.0, c=100.5)
        neutral = _neutral_candle()
        candles = [rechazo, neutral, neutral, neutral]  # age=3 es el primero
    ...
    def test_rechazo_not_fired_when_wick_ratio_below_threshold(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([centered])
    ...
    def test_rechazo_only_up_to_max_age(self) -> None:
        ...
        candles = [rechazo, *neutrals]  # rechazo en age=9 (no debe fire)
        triggers = detect_candle_15m_triggers(candles)
        assert not any("Rechazo" in t["d"] for t in triggers)
...
class TestHammer:
    def test_hammer_fires_with_long_lower_wick_near_lower_bb(self) -> None:
        # body = |100.5 - 100.3| = 0.2
        # lW = min(100.5, 100.3) - 97 = 3.3
        # uW = 101 - max(100.5, 100.3) = 0.5
        # lW > body*2 (0.4) ✓, uW < body*0.5 (0.1) ✗
        # Ajustar: body=0.2, uW debe ser < 0.1. Con h=100.5 y max(o,c)=100.5, uW=0
        hammer = _candle(o=100.5, h=100.5, low=97.0, c=100.3)
        triggers = detect_candle_15m_triggers([hammer], bb_15m=_bb(lower=97.0))
    ...
    def test_hammer_not_fired_when_upper_wick_too_large(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([candle], bb_15m=_bb(lower=97.0))
        assert not any(t["d"] == "Hammer" for t in triggers)
    ...
    def test_hammer_not_fired_when_far_from_lower_bb(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([hammer_shape], bb_15m=_bb(lower=50.0))
        assert not any(t["d"] == "Hammer" for t in triggers)

    def test_hammer_only_fires_at_age_zero(self) -> None:
        hammer = _candle(o=100.5, h=100.5, low=97.0, c=100.3)
        newer = _neutral_candle()
        candles = [hammer, newer]  # age=1
...
class TestShootingStar:
    def test_shooting_star_fires_with_long_upper_wick_near_upper_bb(self) -> None:
        # body = 0.2, uW = 3.3 (long), lW pequeña
        shooting = _candle(o=100.3, h=103.6, low=100.3, c=100.5)
        triggers = detect_candle_15m_triggers([shooting], bb_15m=_bb(upper=103.5))
    ...
    def test_shooting_star_not_fired_without_bb(self) -> None:
        shooting = _candle(o=100.3, h=103.6, low=100.3, c=100.5)
        triggers = detect_candle_15m_triggers([shooting], bb_15m=None)
    ...
    def test_shooting_star_not_fired_when_lower_wick_too_large(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([candle], bb_15m=_bb(upper=103.5))
        assert not any(t["d"] == "Shooting Star" for t in triggers)
...
class TestCombined:
    def test_multiple_rechazos_across_ages_accumulate(self) -> None:
        """5 velas de Rechazo sup deben producir 5 triggers con decay
        decreciente."""
        rechazo = _candle(o=100.0, h=105.0, low=99.0, c=100.5)
        candles = [rechazo] * 5  # ages 0..4
        triggers = detect_candle_15m_triggers(candles)
    ...
    def test_doji_and_rechazo_can_coexist_on_same_candle(self) -> None:
        ...
        triggers = detect_candle_15m_triggers([candle], bb_15m=_bb(upper=105.0))
